...
# --- Title ---
st.markdown("<h1 style='text-align:center;'>🎮 Color Arrangement Challenge</h1>", unsafe_allow_html=True)

# ---------------------------
# Arrangement Mode
# ---------------------------
st.markdown('<p class="white-text">🎮 Choose Arrangement Mode</p>', unsafe_allow_html=True)
...
st.markdown(
    f"<p class='white-text'>🎨 <b>Current Order:</b> {', '.join(st.session_state['current_order'])}</p>",
    unsafe_allow_html=True
)
...
if uploaded_video and st.button("📊 Analyze Video"):
    ...
    if last_frame is None:
        ...
        # ---------------------------
        # Display Stylish Report
        # ---------------------------
        st.markdown("<h2 style='color:#00ffea;'>📊 Challenge Report</h2>", unsafe_allow_html=True)
        st.markdown("<div class='report-box'>", unsafe_allow_html=True)

        for key, value in result_data.items():
            st.markdown(
                f"<p style='font-size:17px; color:#ffffff;'>"
                f"<b style='color:#00ffea;'>{key}:</b> {value}</p>",
                unsafe_allow_html=True
            )
        ...
        st.success("✅ Analysis complete! Keep playing and improving 🎮")
